...
AI Writer Studio 是一个面向政务、国企及企业行政写作场景的桌面端 Web 原型，主要包括 **AI 写作、改写润色、智能审查** 三大功能。产品将传统 Chatbot 的自由对话改造为结构化写作工作台，重点解决长文结构难控制、业务知识难引用和生成内容难追溯的问题。

> 当前仓库为可完整演示的前端交互原型，使用 Mock 数据模拟 AI 与知识库能力，不包含真实大模型、RAG、文件解析及数据库服务。

## 核心功能

智能写作助手主要包括 **AI 写作、改写润色、智能审查** 三大功能，并提供引用溯源、状态保存与内容导出等配套能力。

| 功能类型 | 模块     | 已实现能力                                                     |
| -------- | -------- | -------------------------------------------------------------- |
| 主功能   | AI 写作  | 文章类型与模板配置、概要与大纲生成、目录编辑、分章节生成及进度展示 |
| 主功能   | 改写润色 | 划词选择、扩写、精简、续写、总结、自定义要求及结果替换         |
| 主功能   | 智能审查 | 政治敏感、语法逻辑、格式规范、法律条款四类检查，支持采纳、忽略及批量处理 |
| 配套能力 | 引用溯源 | 正文引用标记、来源信息悬停、文档详情查看与原文切片定位         |
| 配套能力 | 保存导出 | `localStorage` 状态恢复、操作反馈与 HTML 全文导出             |
...
- 接入真实大模型与 RAG 知识库，完善文档解析、检索引用和数据持久化能力。
